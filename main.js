...
// Rachaduras (Certifique-se de ter crack_0.png até crack_9.png)
const crackTextures = [];
...
// Mesh da Rachadura (fica sobreposta ao bloco)
const crackGeo = new THREE.BoxGeometry(1.01, 1.01, 1.01);
const crackMat = new THREE.MeshBasicMaterial({ transparent: true, polygonOffset: true, polygonOffsetFactor: -1 });
const crackMesh = new THREE.Mesh(crackGeo, crackMat);
crackMesh.visible = false;
scene.add(crackMesh);

// ─── 3. JOGADOR E CAIXA DE COLISÃO ───────────────────
...
// ─── 5. SISTEMA DE QUEBRA E INTERAÇÃO ────────────────
...
function updateBreaking() {
    if (breakingBlock) {
        breakProgress += 1.3; // Velocidade da quebra
...
function animate() {
    ...
    // Movimento com colisão independente nos eixos
...
// ─── 7. TOUCH (DIVIDIDO: ESQUERDA=OLHAR / DIREITA=AÇÃO) ───
...
window.addEventListener('pointerdown', e => {
    ...
    if (e.clientX > window.innerWidth / 2) { // LADO DIREITO
