...
// ─── 2. SISTEMA DE ILUMINAÇÃO (IMPORTANTE) ────────────
// Luz ambiente para que nada fique preto absoluto
...
function loadTex(file) {
    const tex = textureLoader.load(`./textures/${file}`, 
        (t) => { console.log(`Carregou: ${file}`); },
        undefined,
        (err) => { console.error(`Erro na textura: ${file}. Verifica se o nome no GitHub está igual!`); }
    );
...
// Materiais com cores de reserva (color) caso a imagem falhe
const mats = {
    grass: [
        new THREE.MeshStandardMaterial({ map: loadTex('grass_side.png'), color: 0x7cfc00 }),
        new THREE.MeshStandardMaterial({ map: loadTex('grass_side.png'), color: 0x7cfc00 }),
        new THREE.MeshStandardMaterial({ map: loadTex('grass_top.png'), color: 0x7cfc00 }),
        new THREE.MeshStandardMaterial({ map: loadTex('dirt.png'), color: 0x8b4513 }),
        new THREE.MeshStandardMaterial({ map: loadTex('grass_side.png'), color: 0x7cfc00 }),
        new THREE.MeshStandardMaterial({ map: loadTex('grass_side.png'), color: 0x7cfc00 })
    ],
    dirt: new THREE.MeshStandardMaterial({ map: loadTex('dirt.png'), color: 0x8b4513 }),
    stone: new THREE.MeshStandardMaterial({ map: loadTex('stone.png'), color: 0x808080 }),
    wood: new THREE.MeshStandardMaterial({ map: loadTex('wood.png'), color: 0xdeb887 }),
    leaf: new THREE.MeshStandardMaterial({ map: loadTex('leaf.png'), color: 0x228b22, transparent: true, alphaTest: 0.5 })
};

// ─── 4. BRAÇO E RACHADURA ─────────────────────────────
const armPivot = new THREE.Group();
// Material MeshBasicMaterial não precisa de luz (garante que o braço apareça)
...
function spawnParticles(pos, color) {
    for (let i = 0; i < 6; i++) {
        ...
        p.userData = { vel: new THREE.Vector3((Math.random()-0.5)*0.1, 0.1, (Math.random()-0.5)*0.1), life: 1.0 };
        scene.add(p);
        particles.push(p);
    }
}
...
function action(place) {
    ...
    if (hits.length > 0 && hits[0].distance < 5) {
        ...
        if (place) {
            ...
            spawnParticles(hit.object.position, 0x888888);
...
function animate() {
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
        ...
        p.userData.vel.y -= 0.005; p.userData.life -= 0.03;
